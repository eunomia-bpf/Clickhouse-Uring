...
SEC("uprobe/libc.so.6:send")
int bpf_send_patch(struct pt_regs *ctx) {
  int fd = PT_REGS_PARM1(ctx);
  const void *buf = (const void *)PT_REGS_PARM2(ctx);
  size_t n = PT_REGS_PARM3(ctx);
  int flags = PT_REGS_PARM4(ctx);
  ...
  int ret = io_uring_submit_send(fd, buf, n, flags);
  if (ret) {
    return 0;
  }
  io_uring_set_linkflag();
...
SEC("uprobe/libc.so.6:recv")
int bpf_recv_patch(struct pt_regs *ctx) {
  int fd = PT_REGS_PARM1(ctx);
  void *buf = (void *)PT_REGS_PARM2(ctx);
  size_t n = PT_REGS_PARM3(ctx);
  int flags = PT_REGS_PARM4(ctx);
  ...
  int ret = io_uring_submit_recv(fd, buf, n, flags);
  if (ret) {
    return 0;
  }
  // io_uring_set_linkflag();
...
int bpf_start_patch(struct pt_regs *ctx) {
  ...
  u32 zero = 0;
  bpf_map_update_elem(&op_count, &index, &zero, BPF_ANY);
  bpf_map_update_elem(&global_seq_num, &index, &zero, BPF_ANY);
