...
int main() {
    ...
    if ((file_fd = open("a.txt", O_RDONLY)) < 0) {
    ...
    while (1) {
        ...
        if (io_uring_submit(&ring) < 0) {
            log_error("io_uring_submit failed for read operation");
            cleanup(sock, file_fd, recv_fd, &ring);
            free(buffer);
            return -1;
        }
        ...
        if (io_uring_wait_cqe(&ring, &cqe) < 0) {
            log_error("Error waiting for cqe for read operation");
            cleanup(sock, file_fd, recv_fd, &ring);
            free(buffer);
            return -1;
        }
        printf("cqe->res: %d\n", cqe->res);
        if (cqe->res <= 0) {
            if (cqe->res < 0) log_error("Read error");
        ...
        printf("[DEBUG] Read %d bytes from offset %ld\n", bytes_read, file_offset);
        printf("[DEBUG] Data read: %.*s\n", bytes_read, buffer);
        ...
        io_uring_prep_send(sqe, sock, buffer, bytes_read, 0);
        ...
        log_debug("Sent data to server");
        printf("[DEBUG] Sent %d bytes to server\n", bytes_read);
        printf("[DEBUG] Data sent: %.*s\n", bytes_read, buffer);
        buffer[bytes_read] = '\0';
    }

    // Handle remaining SQEs
    if (batch_count > 0) {
        io_uring_submit_and_wait(&ring, batch_count);
        for (int i = 0; i < batch_count; i++) {
            if (io_uring_peek_cqe(&ring, &cqe) < 0) {
                log_error("Error peeking for cqe for remaining send operations");
                cleanup(sock, file_fd, recv_fd, &ring);
                free(buffer);
                return -1;
            }
    ...
    // Receive response from server and write to recv.txt using io_uring
    batch_count = 0;
    while (1) {
        ...
        if (io_uring_submit(&ring) < 0) {
            log_error("io_uring_submit failed for recv operation");
            cleanup(sock, file_fd, recv_fd, &ring);
            free(buffer);
            return -1;
        }
        ...
        if (io_uring_wait_cqe(&ring, &cqe) < 0) {
            log_error("Error waiting for cqe for recv operation");
            cleanup(sock, file_fd, recv_fd, &ring);
            free(buffer);
            return -1;
        }
        ...
        if (cqe->res <= 0) {
            if (cqe->res == 0) log_info("Server closed connection");
            else log_error("Receive error");
            io_uring_cqe_seen(&ring, cqe);
            break;
        }
        ...
        // Prepare write operation to recv.txt
        sqe = io_uring_get_sqe(&ring);
        if (!sqe) {
        ...
        if (batch_count >= BATCH_SIZE) {
            ...
            for (int i = 0; i < batch_count; i++) {
                if (io_uring_peek_cqe(&ring, &cqe) < 0) {
                    log_error("Error peeking for cqe during write operation");
                    cleanup(sock, file_fd, recv_fd, &ring);
                    free(buffer);
                    return -1;
                }
        ...
        log_debug("Written data to recv.txt");
        printf("[DEBUG] Written %d bytes to recv.txt\n", bytes_received);
    }
